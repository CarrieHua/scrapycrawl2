...
class BugReportSpider(scrapy.Spider):
    name = "LibreOffice_bugReport"
    
    def start_requests(self):
        url_1 = "https://bz.apache.org/bugzilla/show_bug.cgi?id="
        url_2 = "https://bz.apache.org/bugzilla/show_activity.cgi?id="
        
        yield scrapy.Request(url=url_1+'34966', callback=self.parse)
        yield scrapy.Request(url=url_2+'34966', callback=self.parse_history)
        """
        with open("/home/carrie/scrapyCrawe/mySpider/LibreOffice_reopen_urls.csv") as f:
    ...
    def parse_history(self, response):
        ...
        filename = '/home/carrie/scrapyCrawe/LibreOffice_bugReport/reopen_Apache_2.txt'
    ...
    def parse(self, response):
        page = response.url.split("?")[-1]
        filename_1 = '/home/carrie/scrapyCrawe/LibreOffice_bugReport/reopen_Apache_1.txt'
        with open(filename_1, 'a') as of:
            ...
            of.write('\t')
            #alias
            alias = response.xpath('//*[@id="bz_show_bug_column_1"]/table/tr[3]/td/text()').extract()
            if len(alias) == 0:
                of.write('None')
            else:
                for ali in alias:
            ...
            of.write('\t')
            #version
            version = response.xpath('//*[@id="bz_show_bug_column_1"]/table/tr[8]/td/text()').extract()
            if len(version) == 0:
                of.write('None')
            else:
                for ver in version:
            ...
            of.write('\t')
            #hardware
            hardware = response.xpath('//*[@id="bz_show_bug_column_1"]/table/tr[9]/td/text()').extract()
            if len(hardware) == 0:
                of.write('None')
            else:
            ...
            of.write('\t')
            #importance
            importance = response.xpath('//*[@id="bz_show_bug_column_1"]/table/tr[11]/td/text()').extract()
            if len(importance) == 0:
                of.write('None')
            else:
            ...
            of.write('\t')
            #Assignee
            assigned = response.xpath('//*[@id="bz_show_bug_column_1"]/table/tr[12]/td/span/span/text()').extract()
            if len(assigned) == 0:
                of.write('None')
            else:
                for ass in assigned:
            ...
            of.write('\t')
            #reporter
            reporter = response.xpath('//*[@id="bz_show_bug_column_2"]/table/tr[1]/td/span/text()').extract()
            if len(reporter) == 0:
                of.write('None')
            else:
